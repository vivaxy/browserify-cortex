        ...
        if (!tree[dep]) {
            ...
            request(registry, (error, response, body) => {
                if (!error && response.statusCode === 200) {
                    ...
                    if (~newDependent.repository.indexOf('github')) {
...
const buildBundle = () => {
    ...
    for (let dep in tree) {
        ...
        browserifyInstance.require(dependencePath, {
            expose: dep
        });
    }
    ...
    browserifyInstance.bundle((err, data) => {
        if (err) {
            ...
            fs.writeFile(savePath, data, (err) => {
                if (err) {
                    log.error('fs writeFile error:', err.message);
                    isDebug || usageTracker.send({
                        'fs writeFile error': err.message
                    });
                } else {
